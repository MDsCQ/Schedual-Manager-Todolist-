            ...
            <div class="container-fluid">
                <div class="card shadow">
                    <div class="card-header py-3">
                        <p class="text-primary m-0 fw-bold">日程</p>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                            <table class="table my-0" id="dataTable">
                                ...
                                <tbody>
                                <%
                                    List<Schedual> list= (List<Schedual>) request.getAttribute("schelist");
                                    for(Schedual schedual:list){
                                        out.print("<tr>\n" +
                                                "<td>"+schedual.getTital()+"</td>\n" +
                                                "<td>"+schedual.getAddtime()+"</td>\n" +
                                                "<td>"+schedual.getDeadline()+"</td>\n" +
                                                "<td><form style=\"display: inline-block\" method=\"post\" action=\"/ChangeStateServlet\">\n" +
                                                    "<input type=\"hidden\" name=\"uscheid\" value=\""+schedual.getUscheid()+"\">\n" +
                                                    "<button class=\"btn btn-primary py-0\" type=\"submit\" value=\"完成\">完成</button>\n" +
                                                    "</form>" +
                                                    "<form style=\"display: inline-block\" method=\"post\" action=\"DeleteScheServlet\">\n" +
                                                    "<input type=\"hidden\" name=\"uscheid\" value=\"/"+schedual.getUscheid()+"\">\n" +
                            ...
                            <form class="d-none d-sm-inline-block me-auto ms-md-3 my-2 my-md-0 mw-100 navbar-search" action="/AddScheServlet" method="post">
                                <div class="input-group">
                                    <span><strong>添加任务</strong></span>
                                    <input class="bg-light form-control border-0 small" type="text" placeholder="任务标题" name="tital">
                                    <input class="bg-light form-control border-0 small" type="date" placeholder="截止日期" name="deadline">
